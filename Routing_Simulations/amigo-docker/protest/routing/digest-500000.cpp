...
**********************************************************************************************************************************************
 * 
 * The routing protocol is made up of 2 parts, that occur repeatedly, one after the other: 
 * 
...
//.5 MB:
// n = 2000
// p = 0.00001 (1 in 99825)
// m = 47,926 (5.85KiB)
// k = 17

const uint32_t bloomSize = 47926;
const uint32_t subset_size = 1450;
const uint32_t hashes = 17;

const int maxBufferSize = 500000; // (.5MB) largest buffer that a node can have stored locally; value set from command line
...
class NodeHandler
{
    ...
    public:
        NodeHandler(): messageBuffer(), messageDigest(hashes), recvdMessageDigests(), recvDigestCounts() {}
...
void SendBuffer(Ptr<Socket> socket, Ipv4Address sender, const std::vector<Message>& miniMessageBuffer, uint32_t nodeID, int channel) {
    ...
    std::vector<Message> deserializedMessages = DeserializeMessages(serializedBuffer.data(), serializedBuffer.size());
...
void Receive_MessageBuffer(Ptr<Socket> socket, Ipv4Address sender, Ptr<Packet> packet, int channelID){
    ...
    // NS_LOG_UNCOND("Time: " << Simulator::Now().GetSeconds() << "Recvd Msg: " << nodeID << "From: " <<  sendID << "Messages: " << digest_string);
...
// --------------------------------------------------------------------------------------------------------------------------------------------
void SendDigestChunks(Ptr<Socket> socket, uint32_t nodeID, Ipv4Address remoteIP, int channel){
    ...
    for (size_t i = 0; i < iters; ++i) {
        ...
        // //TEST
        // std::bitset<bloomSize> sent_bs = GetDigestFromBuffer(buffer, packet->GetSize(), i);
        // uint32_t sendID = IpToNode[channel][remoteIP];
        ...
        // NS_LOG_UNCOND("SEND_DIG_CHUNK Time: " << Simulator::Now().GetSeconds() << " Digest Sender Node: " << nodeID << " Digest Receiver Node: " << sendID << "Sent Digest: " << digest_string);
...
void Receive_DigestChunk(Ptr<Socket> socket, Ipv4Address sender, Ptr<Packet> packet, int channelID){
    ...
    if(nodeHandlerArray[nodeID].getDigestCount(sendID) == 1){
        ...
        // NS_LOG_UNCOND("SEND_DIG_CHUNK Time: " << Simulator::Now().GetSeconds() << "Digest Sender Node: " << sendID << "Digest Receiver Node: " << nodeID << "Sent Digest: " << digest_string);
...
void Receive_ProbeResponseFrame(Ptr<Socket> socket, Ipv4Address sender, Ptr<Packet> packet, int c){
    ...
    //send digest
    SendDigestChunks(dataSockets[channel][nodeID], nodeID, sendIP, channel);
}
...
void LogTime(std::string log){
    ...
    LogPacketSends(packetFile, "START");
}
...
int main(int argc, char* argv[]) {
    ...
    std::string outputpath = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/";
    ...
    std::string messageSentTime_outputfile= "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/messageSentTime"+runiter+".csv";
    std::string messageSenderMap_outputfile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/messageSenderMap"+runiter+".csv";
    std::string messageReceivedTime_outputfile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/messageReceivedTime"+runiter+".csv";
    std::string messageReceiverMap_outputfile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/messageReceiverMap"+runiter+".csv";
    std::string metadata_outputfile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/metadata"+runiter+".csv";
    std::string fullness_outputfile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/bufferFullness"+runiter+".csv";
    packetFile = "../protest/results/routing/digest_flooding/"+traffic_model+"/"+mobility_model+"/node"+std::to_string(nodeCount)+"/group"+std::to_string(groupSize)+"/"+std::to_string(maxBufferSize)+"/packets"+runiter+".csv";
